...
namespace LojaProdutos.Services.Estoque
{
    public class EstoqueService : IEstoqueInterface
    {
        ...
        public List<RegistroProdutoModel> ListarRegistros()
        {
            try
            {
                var resultado = from c in _context.ProdutosBaixados.Include(x => x.Produto).Include(y => y.Produto.Categoria).ToList()
                ...
                {
                    var registro = new RegistroProdutoModel()
                    {
